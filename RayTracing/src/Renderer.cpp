...
void Renderer::OnResize(uint32_t width, uint32_t height)
{
	...
	for (int i = 0; i < width; i++)
		m_ImageHorizontalItr[i] = i;

	for (int i = 0; i < height; i++)
		m_ImageVerticalItr[i] = i;
}
...
#if MT
	std::for_each(std::execution::par ,m_ImageVerticalItr.begin(), m_ImageVerticalItr.end(), [this](uint32_t y)
		{
			std::for_each(std::execution::par, m_ImageHorizontalItr.begin(), m_ImageHorizontalItr.end(), [this, y](uint32_t x)
				{
...
Renderer::HitPayload Renderer::MissRay(const Ray& ray)
{
	Renderer::HitPayload payload;
